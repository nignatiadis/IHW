...
```{r plot_ihwRes_rawvsadj, fig.width = 6, fig.height = 3.6, warning = FALSE, out.width = "600px"}
library("ggplot2")
...
As you can see above, the `r class(ihwRes)` object `ihwRes` can be converted to a dataframe that contains the following columns:
...
df <- rbind(data.frame(pvalue = deRes$pvalue, covariate = rank(deRes$baseMean)/nrow(deRes), 
...
For example, if the whole grouping factor is available (e.g., when it was generated by using `groups_by_filter` on the full vector of covariates), then one can apply the `table` function to it to calculate the number of hypotheses per bin. This is then used as an input for the `m_groups` argument. More elaborate strategies might be needed in more complicated cases, e.g., when the full vector of covariates does not fit into memory.
...
```{r}
sim_reported <- subset(sim, sim$pvalue <= reporting_threshold) 
ihw_filt <- ihw(pvalue ~ group, data = sim_reported,
