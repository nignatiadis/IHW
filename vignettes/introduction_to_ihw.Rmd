...
ggplot(as.data.frame(ihw_res), aes(x = pvalue, y = adj_pvalue, col = group)) + geom_point(size = 0.2)
ggplot(subset(as.data.frame(ihw_res), adj_pvalue <= 0.2), aes(x = pvalue, y = adj_pvalue, col = group)) + 
  geom_point(size = 0.2)
```
 
As you can see above, the `r class(ihw_res)` object `ihw_res` can be converted to a `data.frame` that contains the following columns:
