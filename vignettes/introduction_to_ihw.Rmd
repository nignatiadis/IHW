...
IHW then calculates weights for each p-value and also a vector of rejected hypotheses *FIXME what is that?* at the specified significance level. Roughly speaking *FIXME Let's either be precise, or say nothing at all, rather than being diffuse*, the weights allow different prioritization of the individual hypotheses, based on their covariate.

To be more precise, assume we have $m$ different hypothesis tests. Then the hypothesis weights are non-negative random variables (i.e., numbers that in some way will depend on the data *FIXME is it important to talk about 'random variables' here? The audience are general users, not probabilists*) $w_i \geq 0$ such that $\sum_{i=1}^m w_i = m$. We then define the weighted p-values as $P^\text{weighted}_i = \frac{P_i}{w_i}$ and then plug these weighted p-values (instead of the p-values) into the procedure of Benjamini and Hochberg. This means that hypotheses with a higher weight will be more easily rejected than those with lower weight, given the same p-value.

If this covariate also happens *FIXME true from a formal point of view but will be confusing to pragmatic/results oriented users; but you could of course mention that method performance is quite robust against mis-weighting, so in the worst case usually is unchanged, while for good choice of weights, pwer increases* to be informative about the power of the individual tests, then we can actually gain a lot of power compared to an unweighted method (an unweighted method is a weighted method where all tests are assigned weight 1).
...
Basically, a hypothesis with weight > 1 gets prioritized in the testing procedure and the higher the weight the higher the prioritization. On the other hand, a hypothesis with weight equal to 0 cannot be rejected and essentially is filtered out of the procedure. *FIXME redundant with Intro text.. remove there or here?*
