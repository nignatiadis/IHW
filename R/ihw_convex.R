...
ihw.default <- function(pvalues, covariates, alpha,
	...
	if (covariate_type =="ordinal" & is.numeric(covariates)){
		...
		if (nbins == "auto"){
			nbins <- max(1,min(200, floor(length(pvalues)/700))) # rule of thumb..
		}
	...
	if (nbins > 1 & any(m_groups < 200)){
		message("We recommend that you supply (many) more than 200 p-values for meaningful data-driven hypothesis weighting results.")
	}
	...
	folds      <- factor(fill_nas_reorder(res$sorted_folds, nna, reorder_pvalues),levels=fold_levels)
...
ihw_internal <- function(sorted_groups, sorted_pvalues, alpha, lambdas,
	...
	if (!folds_prespecified){
		sorted_folds <- factor(sample(1:nfolds, m, replace = TRUE), levels=1:nfolds)
	}
	...
	for (i in seq_along(levels(sorted_folds))){
      ...
      if (!folds_prespecified || length(dim(m_groups)) == 1){
				...
				m_groups_other_folds_uncollapsed <- m_groups[,colnames(m_groups)!=fold_i,drop=FALSE]
				m_groups_other_folds <- rowSums(m_groups_other_folds_uncollapsed)
			}
		}
	  ws_mat_list[[i]] <- ihw_convex_weights(filtered_split_sorted_pvalues, alpha,
	  ...
	  if  (!return_num_rejection_list && length(lambdas) > 1){
	    ...
	    for (s in seq_len(nsplits_internal)){
	      ...
	      rjs_int <- ihw_internal(filtered_sorted_groups, filtered_sorted_pvalues, alpha,
	                              lambdas, m_groups_other_folds_uncollapsed,
    ...
    if (return_num_rejection_list){
      ...
      for (l in seq_len(n_lambdas)){
        for (i in seq_along(levels(sorted_folds))){
          fold_i = levels(sorted_folds)[i]
          sorted_weights[sorted_folds == fold_i] <- ws_mat_list[[i]][sorted_groups[sorted_folds==fold_i], l]
        }
    ...
    if (!return_num_rejection_list){
      for (i in seq_along(levels(sorted_folds))){
        ...
        sorted_weights[sorted_folds == fold_i] <- ws_mat_list[[i]][sorted_groups[sorted_folds==fold_i], lambda_idx]
        fold_lambdas[i] <- lambdas[lambda_idx]
      }
      sorted_weighted_pvalues <- mydiv(sorted_pvalues, sorted_weights)
      sorted_adj_p <- p.adjust(sorted_weighted_pvalues, method = adjustment_type, n = sum(m_groups))
      rjs   <- sum(sorted_adj_p <= alpha)
      lst <- list(lambda=0.0, fold_lambdas=fold_lambdas, rjs=rjs, sorted_pvalues=sorted_pvalues,
...
ihw_convex_weights <- function(split_sorted_pvalues, alpha, m_groups, m_groups_grenander,
		penalty="total variation", lambdas,
  ...
  max_pval <- 0.9 * max(sapply(split_sorted_pvalues, max))
	if (max_pval < 0.1){
	  split_sorted_pvalues <- lapply(split_sorted_pvalues, function(x) x / max_pval)
  ...
  # still return adjusted pvalues based on the original p-values
	split_sorted_pvalues <- lapply(split_sorted_pvalues, function(x) ifelse(x > 10^(-20), x, 10^(-20)))
	m <- sum(m_groups)
  ...
  ws <- apply(ts, 2, function(x) thresholds_to_weights(x, m_groups))
...
presorted_grenander <- function(sorted_pvalues, m_total=length(sorted_pvalues),
  	...
  	gren <- list(locs = ll$x.knots, Fs = ll$y.knots, fs = ll$slope.knots)
	  if (!quiet) message(paste("Grenander fit with", length(ll$slope.knots), "knots."))
  	gren
}
