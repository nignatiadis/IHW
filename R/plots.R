...
setMethod("plot", signature = "ihwResult",
          function(x,
            ...
            df$weight  <- mapply(function(x, y)
              ws[x, y], df$stratum, df$fold)
...
plot_weights <- function(df, x_axis, scale) {
  switch(scale,
         nominal = {
           ...
           ggplot2::ggplot(df, ggplot2::aes(x = fold, y = weight, fill = fold)) +
         ...
         ordinal = {
           switch(x_axis,
                  group = {
                    ggplot2::ggplot(df, ggplot2::aes(x = stratum, y = weight, col = fold)) +
                      ggplot2::geom_point() +
                      ggplot2::geom_line() +
                      ggplot2::scale_x_continuous(breaks = scales::pretty_breaks())
                  },
                  stop(sprintf(
                    "x_axis = '%s' not implemented.", x_axis
                  )))
         },
...
plot_decisionboundary <- function(x, df, x_axis, scale) {
  ...
  minrk      <- by(xdf, xdf$group, function(v)
    min(v$rkcv))
  pthresh    <- by(xdf, list(xdf$group, xdf$fold), function(v) {
    ...
    if (!any(wh))
      ...
      max(v$pvalue[wh], na.rm = TRUE)
  })

  df$minrk   <- minrk[as.character(df$group)]
  df$pthresh <-
    pthresh[cbind(as.character(df$group), as.character(df$fold))]
