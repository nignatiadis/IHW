...
You will probably be familiar with multiple testing procedures that take a set of p-values and then calculate adjusted p-values. Given a significance level $\alpha$, one can then declare the significant (rejected) hypotheses. In R this is most commonly done with the p.adjust function.

Similarly, IHW (Independent Hypothesis Weighting) is also a multiple testing procedure [@ignatiadis2015data], but beyond the p-values, it also requires a covariate (filter statistic) which is independent of the p-values under the null hypothesis. Therefore the basic input of IHW is the following

* A vector of p-values
* A vector of covariates (filter-statistics), independent of the p-values under the null hypothesis
* A significance level $\alpha \in (0,1)$ at which the False Discovery Rate should be controlled.

IHW then calculates weights for each p-value and also a vector of rejected hypothesis at the specified significance level. Roughly speaking, the weights allow different prioritization of the individual hypotheses, based on their covariate.

To be more precise, assume we have $m$ different hypothesis tests: Then the multiple testing weights are non-negative random variables $w_i \geq 0$ such that $\sum_{i=1}^m w_i = m$. We then define the weighted p-values as $P^\text{weighted}_i = \frac{P_i}{w_i}$ and then plug these weighted p-values (instead of the p-values) into the Benjamini Hochberg procedure. This means that if $w_i > 1$, then it is easier to reject hypothesis $i$.
...
Here we analyze the airway RNA-Seq dataset using DESeq2 [@love2014moderated].
...
This returns an object of the class ihwResult. We can get e.g. the total number of rejections at our pre-specified significance level alpha:
...
In particular, each hypothesis test gets assigned a weight depending on the combination of its assigned fold and its stratum.
...
Finally, IHW contains a convenience function for visualization of the estimated weights:
 ...
 *  For Rank tests  we can use any function that does not depend on the order of arguments [@bourgon2010independent].
